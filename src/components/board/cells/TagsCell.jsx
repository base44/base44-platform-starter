...
const tagSuggestions = [
  { value: "urgent", label: "Urgent", color: "#F06A6A" },
  { value: "bug", label: "Bug", color: "#F1BD6C" },
  { value: "feature", label: "Feature", color: "#5DA283" },
  { value: "marketing", label: "Marketing", color: "#9747FF" },
  { value: "design", label: "Design", color: "#0E2E56" },
];

export default function TagsCell({ value, onUpdate, options }) {
  const currentTags = Array.isArray(value) ? value : []; // value is an array of tag objects like { value: 'urgent', label: 'Urgent', color: '#F06A6A' }
  ...
  const handleCreateNewTag = () => {
    if (
      newTagInput.trim() &&
      !currentTags.find((t) => t.label.toLowerCase() === newTagInput.trim().toLowerCase())
    ) {
