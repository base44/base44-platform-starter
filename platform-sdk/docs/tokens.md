...
The starter adapts its existing database rows for one configured platform/workspace,
retaining its existing expiry column and renewing five minutes before that expiry.
Changing the configured platform requires reconnecting stored links; existing rows
do not record their original host. A new general-purpose adapter should store all
three namespace fields and `renewAt` exactly.
...
See [the compilable adapter example](../examples/usage.ts). No Prisma, Next.js or
