...
All exports come from `platform-sdk/src/index.ts`. Import the directory relatively
until packaging is decided. Types carry the same field documentation in editors.
All methods return promises except construction and `asUser`. No method starts a
socket, stream or background renewal timer. There is no root `apps` module.
...
The SDK does not silently make every new app public. The starter explicitly requests
`public_without_login` and permits embedding to preserve its current behavior.
