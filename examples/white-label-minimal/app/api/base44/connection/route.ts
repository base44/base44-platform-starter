...
export async function POST(request: Request) {
  ...
  if (origin !== allowed)
    return Response.json({ error: 'This request origin is not allowed.' }, { status: 403, headers });
  try {
    ...
    if (error instanceof MissingConfigError)
      return Response.json({ error: 'The Base44 bridge is not configured on this deployment.' }, { status: 501, headers });
    return Response.json({ error: 'Could not update your Base44 connection.' }, { status: 500, headers });
  }
}
