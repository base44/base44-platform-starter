...
export default function BuilderChat({
  ...
  const runtime = useExternalStoreRuntime({
    ...
    onNew: async message => {
      ...
      // The runtime clears the draft on send; preserve it when Base44 rejects
      // or returns an uncertain outcome. The builder controls whether retry is safe.
    ...
    <ThreadPrimitive.Root className="builder-chat">
      <ThreadPrimitive.Viewport aria-label="Conversation" role="region" className="conversation">
        ...
        <ThreadPrimitive.Messages>
          {({ message }) => <MessagePrimitive.Root className={`chat-message ${message.role === 'user' ? 'from-user' : 'from-assistant'}`}>
            <span className="message-avatar" aria-label={message.role === 'user' ? 'You' : 'Assistant'}>
              {message.role === 'user' ? <User size={14} /> : <Bot size={14} />}
            </span>
            <div className="message-body">
              <MessagePrimitive.Parts>
                {({ part }) => {
                  if (part.type === 'text') return <div className="message-bubble">
                    {message.role === 'user' ? <p>{part.text}</p> : <MarkdownTextPrimitive />}
                  </div>;
                  if (part.type !== 'tool-call') return null;
                  const { tool, messageId } = part.artifact as { tool: ToolCall; messageId: string };
                  return (tool.status === 'waiting_for_user_input' || tool.waiting_on?.kind) && appId
                    ? <Question key={`${part.toolCallId}:${tool.status}`} tool={tool} messageId={messageId} appId={appId} disabled={questionsDisabled} onSubmit={onAnswer} />
      ...
      <ComposerPrimitive.Root className="composer">
        <ComposerPrimitive.Input
          ...
          maxLength={16000} rows={1} maxRows={4}
        />
