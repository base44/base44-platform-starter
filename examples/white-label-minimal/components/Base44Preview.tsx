...
export default function Base44Preview(props: Base44PreviewProps) {
...
function PreviewSession({ appId, live = false, title, staticUrl, screenshotUrl, loadPreview, showControls = true, onStatusChange }: Base44PreviewProps) {
  ...
  useEffect(() => {
    ...
    async function connect(recovering = false) {
      ...
      try {
        const result = await loadPreviewRef.current(appId);
        if (cancelled) return;
        ...
        if (cancelled) return;
        blocked = typeof err === "object" && err !== null && "status" in err && (err.status === 401 || err.status === 403);
  ...
  return <div className="preview-frame">
    ...
    {showControls && <div className="preview-controls">
